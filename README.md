...
> 🚩 このOSには**4つのCTFフラグ**（偵察・暗号・ログ解析・リバーシング）が隠されています。`flags` で進捗を確認、1つ見つけるとROOTが解放、全制覇すると……？ ネタバレはこのREADMEには書きません。
...
- **本当に打てるターミナル** — コマンド履歴（↑↓）、30近いコマンド、偽ファイルシステム
- **4フラグのミニCTF** — 偵察／ROT13暗号／ログのhexペイロード解析／`strings`でバイナリ解析。スコアボード・隠しROOT画面・全制覇演出つき
